...
- **PWA with Native Push Notifications:** Full PWA implementation (v2.0) with offline support, advanced caching, and native-like push notifications via Web Push API and VAPID protocol. Push notifications work even when the app is closed or in background, appearing in the device's notification center with icon, vibration, and sound like WhatsApp. Notifications are sent for key events (new messages, offers, order validation, account activation) with deep links to specific pages. Installation uses native browser beforeinstallprompt event only (no custom buttons). Service Worker v2.0 includes updateViaCache: 'none' for instant updates and 30-minute update checks. VAPID keys externalized to Replit Secrets for security.
