...
## Recent Changes
- **2025-10-30**: Implemented automatic expiration system for archived coordination requests. When a coordinator archives a request (via coordination status management), the request automatically changes status to "expired", becoming invisible to transporters and moving to client's "Terminées" tab with orange badge and notice message "Cette commande a été marquée comme expirée par l'équipe CamionBack" (optionally including coordinator's reason). Backend: Modified `updateCoordinationStatus()` in DbStorage to detect "archives" category statuses and set `status = 'expired'` automatically. Simplified `getOpenRequests()` to naturally exclude expired requests (since status changed from "open" to "expired"). Frontend: Updated client dashboard completed requests filter to include `status === "expired"`, added conditional badge/message display with dark mode support. Provides coordinator team with powerful workflow management tool to remove problematic/outdated requests from transporter view while maintaining transparency with clients.
- **2025-10-30**: SÉCURISATION COMPLÈTE backend CamionBack - 72 endpoints protégés et sanitizés. Infrastructure : (1) server/security-utils.ts avec sanitizeUser/maskPhoneNumber/sendUser pour supprimer passwordHash et masquer numéros (+2126•••••534), (2) server/session-config.ts avec PostgreSQL sessions HttpOnly/Secure/SameSite 7j, (3) server/auth-middleware.ts avec requireAuth/requireRole/getCurrentUser. Protection endpoints : 7 auth (register/login créent sessions, /api/auth/me session-based, legacy /api/auth/me/:userId avec ownership), 38 admin (requireAuth + requireRole['admin'], sanitization complète), 20 coordinator (requireAuth + requireRole['admin','coordinateur']), 7 users (requireAuth + ownership strict, GET /api/users admin only). Corrections critiques : /api/admin/validate-driver retournait passwordHash → sendUser(), /api/diagnostic/database-stats PUBLIC exposait transporters → requireAuth admin + sanitization, endpoints PWA test → requireAuth. Résultat : ZÉRO passwordHash exposé, masquage numéros, authentification obligatoire partout. PROCHAINES ÉTAPES : rate limiting brute-force, security_logs table, migration frontend localStorage→sessions (CRITIQUE restaurer UI), tests e2e sécurité.
- **2025-10-30**: Fixed coordination status filtering system to use dynamic status configuration from database instead of hardcoded values. Modified `getCoordinationEnActionRequests()`, `getCoordinationPrioritairesRequests()`, and `getCoordinationArchivesRequests()` in DbStorage to query `coordination_statuses` table and fetch active statuses by category dynamically. Previously used hardcoded arrays (`['client_injoignable', 'infos_manquantes', ...]`) which prevented newly created custom statuses from appearing in filtered views. Now supports full admin-configurable status management - any new status created via admin UI automatically appears in correct coordination workflow tab. Fixed localStorage key bug in CoordinationStatusManagement component (changed from `"user"` to `"camionback_user"`) which caused 403 errors when creating/editing statuses. Also added `enabled: !!user?.id` condition to coordination status queries in CoordinatorDashboard to prevent premature query execution.
- **2025-10-30**: Modified SMS notification system to send alerts to clients for EVERY new offer received (not just the first one). Renamed `sendFirstOfferSMS` to `sendNewOfferSMS`, updated message to indicate "nouvelle proposition tarifaire", removed the `allOffers.length === 1` condition and `smsSent` flag check. Now every transporter offer submission triggers an SMS to the client via Infobip. Also removed all emojis from infobip-sms.ts console logs and SMS messages (replaced with plain text and [Infobip] prefix).
