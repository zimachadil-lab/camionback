...
## Recent Changes
- **2025-10-30**: Implémentation correctifs de sécurité critiques suite alerte externe. Créé infrastructure de sécurité complète : (1) server/security-utils.ts avec sanitizeUser(), maskPhoneNumber(), sendUser/sendPayload() pour retirer passwordHash et masquer numéros (+2126•••••534), (2) server/session-config.ts avec express-session + PostgreSQL store + cookies HttpOnly/Secure/SameSite pour sessions sécurisées 7 jours, (3) server/auth-middleware.ts avec requireAuth(), requireRole(), getCurrentUser() pour RBAC. Sécurisé endpoints authentification : register/login créent req.session.userId, nouveau endpoint GET /api/auth/me basé session (sans param userId), endpoint logout destroy session, protection ownership sur endpoint legacy /api/auth/me/:userId. Tous endpoints auth retournent maintenant users sanitizés (passwordHash JAMAIS exposé). PROCHAINES ÉTAPES : appliquer requireAuth/requireRole sur TOUS endpoints admin/coordinator, sanitizer toutes réponses API, implémenter rate limiting brute-force, migrer frontend localStorage vers sessions cookies, créer security_logs table.
