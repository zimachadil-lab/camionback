...
- **Intelligent Transporter Recommendation System:** Client dashboard features two action buttons on non-accepted transport request cards: (1) "Recommandations transporteurs" button that opens a dialog displaying up to 5 intelligently prioritized transporters based on: empty returns on the same route (highest priority), recent platform activity, high satisfaction ratings (≥4.0 stars), and random selection from remaining validated transporters. Each recommended transporter card shows rating, trip count, city, and priority badges (Retour à vide, Actif récemment, Bien noté) with a "Contacter" button that initiates ephemeral messaging and records client-transporter contact in the database, triggering admin notifications. (2) "Demander l'aide d'un coordinateur" button that opens WhatsApp Web with pre-filled message to coordinator (+212664373534) containing request reference ID and route information. Backend includes GET `/api/recommendations/:requestId` for prioritized transporter lists, POST `/api/client-transporter-contacts` for contact tracking, and `clientTransporterContacts` database table (requestId, clientId, transporterId, contactType, createdAt).
