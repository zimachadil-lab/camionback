...
- **Admin Demandes Offer Counter (October 2025):** Admin dashboard "Demandes" view displays offer count for each transport request, enriched via backend aggregation. Format: "X offre(s)" shown on each request card. Provides quick visibility into transporter interest levels. Note: Current implementation may have N+1 query performance at scale; future optimization with SQL aggregation recommended.
- **Admin Transporter Permanent Deletion (October 2025):** Admin "Transporteurs" view includes Trash2 icon for permanent transporter account deletion with complete CASCADE DELETE across all related tables (offers, messages, notifications, ratings, contracts, reports, emptyReturns, smsHistory, clientTransporterContacts, credentials). Phone number is freed for reuse. Deletion available for both validated and pending transporters with native confirmation dialog. Schema updated to include CASCADE DELETE on clientTransporterContacts foreign keys.
- **Multimedia Messaging System - Photos & Videos (October 2025):** Full photo and video sharing capabilities in chat conversations. Extended chatMessages schema with fileName, fileSize, and messageType extended to support 'photo' and 'video' in addition to existing 'text' and 'voice'. Backend endpoint POST `/api/messages/upload-media` with Multer (10MB limit, mime type filtering for images and videos), base64 encoding, and automatic type detection. Frontend ChatWindow includes Image icon upload button, clickable photo thumbnails with fullscreen view on click, native video player with controls, loading states with spinner, file size validation, and error handling. Storage note: Base64 media storage (10MB max per file) may cause database bloat at scale; future migration to object storage (S3/GCS) with signed URLs recommended.
- **CamioMatch Enhanced Filtering (October 2025):** CamioMatch recommendation algorithm now filters out blocked transporters (accountStatus !== 'active') from all recommendation views, ensuring only active validated transporters appear in client-facing suggestions.
