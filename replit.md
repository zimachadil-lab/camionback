...
- **CamioMatch - Intelligent Transporter Matching System:** Client dashboard features two action buttons on non-accepted transport request cards: (1) "⚡ CamioMatch" button that opens a Tinder-style swipe interface displaying up to 5 intelligently prioritized transporters. Features include: animated truck loading indicator (@keyframes drive), large centered cards with transporter photos/ratings/details, navigation arrows and position indicators, priority badges with Lucide icons (Target for empty returns, Zap for recent activity, Star for high ratings ≥4.0), circular action buttons (red X to skip, turquoise gradient MessageSquare to contact), and hover-elevate interactions following design guidelines. Prioritization algorithm: empty returns on same route (highest), recent platform activity (24h), high satisfaction ratings (≥4.0 stars), random from remaining validated transporters. Each contact is recorded in database and triggers admin notifications. (2) "Coordinateur" button that opens WhatsApp Web with pre-filled message to coordinator (+212664373534) containing request reference ID and route information. Backend includes GET `/api/recommendations/:requestId` for prioritized transporter lists, POST `/api/client-transporter-contacts` for contact tracking, and `clientTransporterContacts` database table (requestId, clientId, transporterId, contactType, createdAt). Design: turquoise gradient (from-[#17cfcf] to-[#13b3b3]), rounded-xl borders, shadow effects, no emoji (Lucide icons only), hover-elevate/active-elevate-2 utilities.
