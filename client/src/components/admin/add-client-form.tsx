...
export function AddClientForm({ open, onClose, onSuccess }: AddClientFormProps) {
  ...
  const onSubmit = async (data: ClientFormData) => {
    try {
      ...
      // Step 1: Register the client with PIN
      ...
      // Step 3: Complete profile with name, city, and optional photo
      ...
      toast({
        variant: "destructive",
        title: "Erreur",
        description: error.message || "Échec de l'ajout du client",
      });
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl">Ajouter un client</DialogTitle>
        </DialogHeader>
        <Form {...form}>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger data-testid="select-client-city">
                    ...
                    <div className="border-2 border-dashed rounded-lg p-6 text-center hover-elevate cursor-pointer">
                      ...
                      <label htmlFor="client-photo-upload" className="cursor-pointer">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {value?.name || "Cliquez pour ajouter une photo (optionnel)"}
                        </p>
                      </label>
                    </div>
                  </FormControl>
