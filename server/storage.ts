...
export class DbStorage implements IStorage {
  ...
  async getCoordinatorActiveRequests(): Promise<any[]> {
    ...
    return results.map(row => ({
      ...
      publishedForMatchingAt: row.publishedForMatchingAt,
      departureAddress: row.departureAddress,
      arrivalAddress: row.arrivalAddress,
      distance: row.distance,
      handlingRequired: row.handlingRequired,
      departureFloor: row.departureFloor,
      departureElevator: row.departureElevator,
      arrivalFloor: row.arrivalFloor,
      arrivalElevator: row.arrivalElevator,
