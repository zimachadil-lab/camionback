...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // DEPRECATED: Old requalify endpoint - replaced by /api/coordinator/requests/:requestId/cancel-and-requalify
  // Kept for backwards compatibility but should not be used
  // TODO: Remove after confirming all clients are using the new endpoint
  ...
  app.get("/api/admin/requests", requireAuth, requireRole(['admin']), async (req, res) => {
    try {
      ...
      const requestsWithStats = requestsData.rows.map((row: any) => ({
        ...
        transporterAmount: row.transporter_amount ? parseFloat(row.transporter_amount) : null,
        platformFee: row.platform_fee ? parseFloat(row.platform_fee) : null,
        clientTotal: row.client_total ? parseFloat(row.client_total) : null,
